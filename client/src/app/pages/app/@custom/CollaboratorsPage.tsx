...
function statusBadge(status: CollaboratorStatus) {
  if (status === 'active') return <Badge variant="success">Active</Badge>
  if (status === 'pending') return <Badge variant="warning">Pending</Badge>
  return <Badge variant="destructive">Revoked</Badge>
}

function roleBadge(role: CollaboratorRole) {
  const styles: Record<CollaboratorRole, string> = {
...
function EmptyCollaborators({ onInvite }: { onInvite: () => void }) {
  return (
...
export function CollaboratorsPage() {
  ...
  async function handleRoleChange(collaborator: Collaborator, role: CollaboratorRole) {
  ...
  async function handleRemove() {
    ...
    try {
      ...
      setCollaborators((prev) =>
        prev.map((c) =>
          c.id === removeTarget.id ? { ...c, status: 'revoked' as CollaboratorStatus } : c
        )
      )
