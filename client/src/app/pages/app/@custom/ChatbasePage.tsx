...
type ChatbaseConfig = {
  ...
  theme_color: string
  visibility: 'always' | 'logged-in' | 'hidden'
}
...
type ChatbaseSettings = {
  ...
  api_key: string
  api_key_set: boolean
  config: Partial<ChatbaseConfig>
}

const defaultConfig: ChatbaseConfig = {
...
export function ChatbasePage() {
  ...
  const [settings, setSettings]       = useState<ChatbaseSettings | null>(null)
  ...
  const [config, setConfig]           = useState<ChatbaseConfig>(defaultConfig)
  ...
  useEffect(() => {
    api.get<{ settings: ChatbaseSettings | null }>('/chatbase/settings')
      .then(({ settings: s }) => {
        if (s) {
          setSettings(s)
  ...
  async function handleSave(e: FormEvent) {
    ...
    setTestResult(null)
    try {
      const body: Record<string, unknown> = { chatbot_id: chatbotId, config }
      if (apiKey) body.api_key = apiKey
      const { settings: updated } = await api.post<{ settings: ChatbaseSettings }>(
                  ...
                  <form onSubmit={handleSave} className="space-y-5">
                    ...
                    <FormField label="Widget Position">
                      <select
                        ...
                        onChange={(e) =>
                          setConfig((c) => ({ ...c, position: e.target.value as ChatbaseConfig['position'] }))
                        }
                    ...
                    <FormField label="Visibility">
                      <select
                        ...
                        onChange={(e) =>
                          setConfig((c) => ({ ...c, visibility: e.target.value as ChatbaseConfig['visibility'] }))
                        }
                        className="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
                      >
